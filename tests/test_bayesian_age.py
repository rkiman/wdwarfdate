...
def test_get_isochrone_model():
    for feh in ['p0.00', 'm4.00', 'm1.00', 'p0.50']:
        for vvcrit in ['0.0', '0.4']:
            ...
            N = len(initial_mass)
            delta_mass = [initial_mass[i + 1] - initial_mass[i]
                          for i in range(N - 1)]
            delta_age = [log_ms_age[i + 1] - log_ms_age[i]
                         for i in range(N - 1)]
            delta_mass, delta_age = np.array(delta_mass), np.array(delta_age)
            e1 = "Outlier in mass model feh:{}, vvcrit:{}".format(feh, vvcrit)
            e2 = "Outlier in age model feh:{}, vvcrit{}".format(feh, vvcrit)
...
def test_ln_posterior_prob():
    ...
    post_true = ln_posterior_prob(params, teff0, e_teff0, logg0, e_logg0,
                                  models0)
