...
def get_idx_sample(posterior_grid):
    ...
    sample_idx = np.array(random.choices(idx_list[mask_nan], posterior_flat[mask_nan], k=2048))
    return sample_idx
...
def get_other_params(mi_sample, tott_sample, delta_m_sample, models):
    ...
    log10_ms_age_sample = f_ms_age(np.log10(mi_sample))
    log10_cool_age_sample = np.log10(10 ** tott_sample - 10 ** log10_ms_age_sample)
    return mf_true_sample, log10_ms_age_sample, log10_cool_age_sample


def calc_posterior_grid(teff_i, e_teff_i, logg_i, e_logg_i, models, n_mi,
                        n_log10_tott, n_delta, min_mi, max_mi,
    ...
    ttot = np.linspace(10 ** min_log10_ttot, 10 ** max_log10_ttot,
                       n_log10_tott)
    log10_ttot = np.log10(ttot)
    log10_mi = np.log10(mi)
    delta_m = np.linspace(-0.1, 0.1, n_delta)

    # Defining grid using axis
    grid = np.meshgrid(log10_mi, log10_ttot, delta_m, indexing='ij')

    # Defining grid version of axis
    log10_mi_grid = grid[0]
    log10_ttot_grid = grid[1]
    delta_m_grid = grid[2]

    # Setting log priors
    log_prior_mi = (-1.3) * log10_mi_grid * np.log(10)  # IMF prior on mi
    log_prior_ttot = log10_ttot_grid * np.log(10)  # Constant SFH prior in ttot
    ...
    log10_tcool_grid = np.copy(log10_ttot_grid) * np.nan
    diff = 10 ** log10_ttot_grid - 10 ** log10_tms_grid
    mask = diff > 0
    log10_tcool_grid[mask] = np.log10(diff[mask])
    ...
    mask_nan = np.isnan(log10_mf_true_grid + log10_tcool_grid)  # Make the interpolation of teff
    ...
    log_post = log_likelihood + log_prior_ttot + log_prior_mi + log_prior_delta_m
    ...
    # Compile results
    params = [10 ** log10_mi, log10_ttot, delta_m]
    params_grid = [10 ** log10_mi_grid, log10_ttot_grid, delta_m_grid]
