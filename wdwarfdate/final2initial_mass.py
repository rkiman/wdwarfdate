...
def calc_initial_mass(model_ifmr,final_mass_dist,n_mc):
    ...
    if(model_ifmr == 'Cummings_2018'):
        ...
        for final_mass_dist_i in final_mass_dist:     
            initial_mass_dist_i = np.ones(n_mc)*np.nan
            for j in range(n_mc):
                if((0.5588 <= final_mass_dist_i[j]) and (final_mass_dist_i[j] <= 0.867)):
                    initial_mass_dist_i[j] = (final_mass_dist_i[j] - 0.331)/0.134
                elif(0.867 < final_mass_dist_i[j]):
                    initial_mass_dist_i[j] = (final_mass_dist_i[j] - 0.679)/0.047
                else:
                    0
    ...
    #Remove all the values that are lower than the limit of initial mass in isochrones
