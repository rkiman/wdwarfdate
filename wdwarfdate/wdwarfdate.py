...
def calc_wd_age(teff0, e_teff0, logg0, e_logg0, method,
    ...
    if method == 'bayesian':
        ...
        for teff0_i, e_teff0_i, logg0_i, e_logg0_i, c_i in zip(teff0, e_teff0,
                                                               logg0, e_logg0,
                                                               comparison):
            print(f'Running teff:{teff0_i} logg:{logg0_i}')
            check_ranges(teff0_i, logg0_i, model_wd)
...
def calc_bayesian_wd_age(teff0, e_teff0, logg0, e_logg0,
    ...
    if datatype == 'yr':
        results = calc_percentiles(10 ** ln_ms_age, 10 ** ln_cooling_age,
                                   10 ** ln_total_age,
                                   initial_mass, final_mass, high_perc,
                                   low_perc, datatype=datatype)
    elif datatype == 'Gyr':
        results = calc_percentiles((10 ** ln_ms_age) / 1e9,
    ...
    if plot:
        if datatype == 'yr':
            plot_distributions(10 ** ln_ms_age, 10 ** ln_cooling_age,
                               10 ** ln_total_age, initial_mass, final_mass,
...
def calc_wd_age_fast_test(teff0, e_teff0, logg0, e_logg0, n_mc, model_wd, feh,
                          vvcrit,
                          model_ifmr, high_perc, low_perc, datatype, comparison,
                          path,
